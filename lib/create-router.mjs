...
function sparqlistEnv() {
  return Object.entries(process.env)
    .filter(([key]) => key.startsWith('SPARQLIST_'))
...
export default function createRouter(
  ...
  function serverLocalUrl(req) {
    const prot = req.protocol;
    const addr = req.socket.address();
    const port = addr.port;

    if (addr.family === 'IPv6') {
      return `${prot}://[${addr.address}]:${port}${pathPrefix}api/`;
    } else if (addr.family === 'IPv4') {
      return `${prot}://${addr.address}:${port}${pathPrefix}api/`;
    } else {
      throw new Error(`unsupported address family '${addr.family}'`);
    }
  }
  ...
  function handleFormat(req, res, next) {
    if (req.params.format) {
      req.headers.accept = mime.getType(req.params.format);
    }
  ...
  async function handleSparqlet(req, res) {
    ...
    const data = await req.sparqlet.execute(
      params,
      req.header('accept'),
      sparqlistEnv(),
      serverLocalUrl(req)
  ...
  if (timeoutMillisec > 0) {
    router.use('/api', timeout(timeoutMillisec));
    router.use('/trace', timeout(timeoutMillisec));
  }

  router.get('/api/:sparqlet_id', loadSparqlet, handleSparqlet);

  router.get(
    '/api/:sparqlet_id.:format',
    loadSparqlet,
    handleFormat,
    handleSparqlet
  );

  router.post('/api/:sparqlet_id', loadSparqlet, handleSparqlet);
