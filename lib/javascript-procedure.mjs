...
export default class JavaScriptProcedure {
  ...
  async execute(context, logEntries, accept, baseUrl) {
    const script = this.data.data;
    const logs = [];

    const relativeUrlAwareFetch = (resource, init) => {
      return fetch(new URL(resource, baseUrl), init);
    };
    ...
    try {
      ...
      if (typeof obj === 'function') {
        ...
        contentType = format.includes('/') ? format : mime.getType(format);
      }
    } finally {
