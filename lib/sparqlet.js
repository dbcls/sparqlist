...
module.exports = class SPARQLet {
  ...
  async execute(params, traceMode) {
    ...
    const {traces} = await this.procedures.reduce(async (acc, proc) => {
      const {traces, context, previous} = await acc;
      const ctx = JSON.parse(JSON.stringify(context));
      const {results, logEntries} = await Procedure.create(proc).execute(ctx, previous);
    ...
    }, {traces: [], previous: defaultResolvedParams, context: {params: defaultResolvedParams}});
