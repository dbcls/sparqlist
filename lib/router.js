...
module.exports = (repositoryPath, adminPassword) => {
  ...
  router.get('/api/:sparqlet_id.:format?', loadSparqlet, async (req, res) => {
  ...
  router.get('/trace/:sparqlet_id.:format?', loadSparqlet, async (req, res) => {
    const data = await req.sparqlet.execute(req);
    res.set('Access-Control-Allow-Origin', '*');
    if (data.error) {
      res.status(500);
    }
    // NOTE include final Content-Type in trace data
    res.json(data);
  });
