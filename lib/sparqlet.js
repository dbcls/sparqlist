...
module.exports = class SPARQLet {
  ...
  async execute(req) {
    const defaultResolvedParams = this.defaultResolvedParams(req.params);
    const {error, traces} = await this.procedures.reduce(async (acc, proc) => {
      ...
      const accepts = isFinal ? req.accepts.bind(req) : undefined;
      const ctx = JSON.parse(JSON.stringify(context));

      let results, contentType, currentError;
      const logEntries = [];
      try {
        const r = await Procedure.create(proc).execute(ctx, logEntries, accepts);
        results = r.results;
